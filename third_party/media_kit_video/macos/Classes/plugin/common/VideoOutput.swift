...
public class VideoOutput: NSObject {
  ...
  private let metalFXScale: Double
  ...
  init(
    ...
    enableMetalFX = configuration.enableMetalFX
    metalFXScale = configuration.metalFXScale
  ...
  private func _init() {
    ...
    if enableMetalFX && TextureSWMetalFX.supported {
    ...
    } else if enableMetalFX {
      NSLog("VideoOutput: warning: MetalFX requested but not supported on this Apple device or OS version.")
      texture = SafeResizableTexture(
        TextureSW(
          handle: handle,
          // Use `weak self` to prevent memory leaks
