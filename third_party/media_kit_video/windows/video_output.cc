...
typedef int (*ArtCnnProcessFrameType)(uint8_t*, int, int, int);
static ArtCnnProcessFrameType g_artcnn_process_frame = nullptr;
static std::once_flag g_artcnn_init_flag;
static bool g_artcnn_enabled_global = false;

extern "C" __declspec(dllexport) void media_kit_video_set_artcnn_enabled(bool enabled) {
    g_artcnn_enabled_global = enabled;
}
...
void VideoOutput::Render() {
  if (texture_id_) {
    ...
    if (pixel_buffer_ != nullptr) {
      ...
      // --- ARTCNN ONNX INJECTION ---
      if (g_artcnn_enabled_global) {
          std::call_once(g_artcnn_init_flag, []() {
              HMODULE hMod = LoadLibraryW(L"anivault_core.dll");
              if (hMod) {
                  g_artcnn_process_frame = (ArtCnnProcessFrameType)GetProcAddress(hMod, "artcnn_process_frame");
              } else {
                  std::cout << "media_kit: Failed to load anivault_core.dll!" << std::endl;
              }
          });
          
          if (g_artcnn_process_frame) {
              // SW_RENDERING_PIXEL_BUFFER_SIZE bounds the max memory we allocated. 
              // The FFI receives the pointer to our buffer.
              g_artcnn_process_frame(pixel_buffer_.get(), pitch * size[1], size[0], size[1]);
          }
      }
    }
    try {
